...
**LLM:** GLM-5 via Friendli Serverless Endpoints
...
```
competitors.csv  →  SerpAPI (Google News)  →  GLM-5 on Friendli  →  Slack webhook
```
...
3. Sends all raw search results to GLM-5 on Friendli Serverless Endpoints to synthesize a structured executive brief
